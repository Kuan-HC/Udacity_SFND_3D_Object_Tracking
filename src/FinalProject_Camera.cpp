...
int main(int argc, const char *argv[])
{
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex += imgStepWidth)
    {
        ...
        detectObjects(dataBuffer.last().cameraImg, dataBuffer.last().boundingBoxes, confThreshold, nmsThreshold,
        ...
        dataBuffer.last().lidarPoints = move(lidarPoints);
        ...
        cv::Mat imgGray;
        cv::cvtColor(dataBuffer.last().cameraImg, imgGray, cv::COLOR_BGR2GRAY);
        ...
        dataBuffer.last().keypoints = move(keypoints);
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            for (auto it1 = dataBuffer.last().bbMatches.begin(); it1 != dataBuffer.last().bbMatches.end(); ++it1)
            {
                // find bounding boxes associates with current match
                BoundingBox *prevBB, *currBB;
                for (auto it2 = dataBuffer.last().boundingBoxes.begin(); it2 != dataBuffer.last().boundingBoxes.end(); ++it2)
                {
                ...
                for (auto it2 = dataBuffer.secondLast().boundingBoxes.begin(); it2 != dataBuffer.secondLast().boundingBoxes.end(); ++it2)
                {
