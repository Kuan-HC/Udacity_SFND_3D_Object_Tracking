    ...
    void computeTTCLidar(std::vector<LidarPoint> &lidarPointsPrev, std::vector<LidarPoint> &lidarPointsCurr, double frameRate, double &TTC)
{
    /**
     * @brief:
     * Use the normal distribution Oto filter out outliers. To identify outliers using the normal distribution, we can use the standard deviation, 
     * which measures how much the values vary from the mean.
